...
showTasks(); // calling showTasks function and save localStorage when reloading page
...
function showTasks() {
  ...
  // if arr length is greater than 0
  if(listArr.length > 0) {
      deleteAllBtn.classList.add("active"); // active the clear all btn
  }else {
    deleteAllBtn.classList.remove("active"); // unacives the clear all btn
  }
...
deleteAllBtn.onclick = () => {
  ...
  localStorage.setItem("New Todo", JSON.stringify(listArr)); // transforming js object into a json string
  showTasks(); // calling showTasks funct
}